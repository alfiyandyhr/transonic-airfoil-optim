...
int fval(int nx, int nf, int ng, double *x, double *f, double *g)
{
  ...
  /* fp = fopen("eval.in", "w"); */
  fp = fopen("Initial_Sampling/preeval.in", "w");
  fprintf(fp, "%d %d %d %d\n", 1, nx, nf, ng);
  for (n=0; n<nx; n++)
    fprintf(fp, "%lf ", x[n]);
  fclose(fp);
  ...
  /* fp = fopen("eval.out", "r"); */
  fp = fopen("Initial_Sampling/preeval.out", "r");
  for (n=0; n<nx; n++)
    fscanf(fp, "%lf", &dmy);
  for (n=0; n<nf; n++)
    fscanf(fp, "%lf", &f[n]);
...
int main(void)
{
  ...
  remove("Initial_Sampling/preeval.in");
  remove("Initial_Sampling/preeval.out");
  ...
  /* output data */
  fp = fopen("Initial_Sampling/dv_lhs.dat", "w");
  for (n=0; n<smp; n++) {
    for (m=0; m<ndv; m++)
