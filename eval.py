...
def evaluate_aero(gen, mesh, CFD, done, diverge):
	"""
	...
	if CFD:	
		for i in range(pop_size):
			if i not in done and i not in diverge:
...
def evaluate_y_diff(array):
	...
	between 2 pairs of points near T.E.
	...
	"""
	y_diffs = np.zeros((2,1))
	y_diffs[0] = array[n_var+1] - array[1]
	y_diffs[1] = array[n_var] - array[2]

	return y_diffs

def evaluate(gen, pop, mesh, CFD, done, diverge):
	"""
	...
	if mesh or CFD:
		evaluate_aero(gen, mesh, CFD, done, diverge)

	pop_eval = np.zeros((pop_size,3)) #for objective functions
	for i in range(pop_size):
		data = pd.read_csv('Solutions/gen_'+str(gen)+'/random_design'+str(i)+'/history.csv',
			header=0, usecols=['       "CD"       ','       "CL"       ','       "CMz"      '])
		interest = np.array([data.iloc[len(data)-1]])
		pop_eval[i] = interest
	pop_eval[:,1] = - pop_eval[:,1]
	pop_eval[:,2] = - (pop_eval[:,2] - ref_CMz)
	...
	pop.set('F',pop_eval[:,0:2])
	pop.set('G',pop_eval[:,2:6])
	set_cv(pop)

	pop_eval = np.concatenate((pop_eval,pop.get('CV')),axis=1)
...
# evaluate(gen=3, pop=None, mesh=False, CFD=True, done=[-1], diverge=[-1])